...
- **文章列表 / 详情**：Markdown 渲染、代码高亮、代码 / 文本 / 脚本块一键复制（右上角常驻，随主题变色，新内容自动生效）、Mermaid 图示、目录 TOC、上一篇 / 下一篇、难度标签、浏览量统计（登录用户 24h 去重）
...
- **正文预览（预览 = 正文）**：编辑器与投稿预览都调用服务端同一套 Markdown 渲染 + 消毒规则（`POST /api/admin/articles/preview`、`POST /api/user/uploads/preview`），预览弹窗右侧带目录，表格 / 删除线 / 任务列表 / 代码高亮 / Mermaid 流程图 / 复制按钮与发布后的正文完全一致
- **学习分类管理**：维护学习专题的分类与板块
- **分类管理、标签管理、用户管理**
- **访问申请审核、用户投稿审核**
- **公告管理、站内通知发布**
